...
<figure class="left"> <img src="/static/images/methodology-status-plot-revised-65-85.png" style="width:450px;"> <figcaption><small><p><b>Figure 4.</b> Chart showing the breakdown between verified (22%), google verified (66%), and locations that could not be verified (12%).</p></small></figcaption> </figure> 
...
* What it does: The interactive map allows you to visualize the geographic distribution of LGBTQ spaces across the United States from 1965 to 2003. You can filter locations by year, state, amenity feature, and location type. The map uses Mapbox for rendering and allows you to zoom in on specific regions or cities to explore local LGBTQ geographies.
...
* Best used for:
    ...
    * Finding specific establishments and learning what Damron said about them
* Limitations:
    * The map shows the locations as they were listed in the Damron guides, not necessarily all LGBTQ spaces that existed at the time. Damron's coverage was incomplete and reflected the biases and limitations of his network of informants.
    * Locations with unclear addresses appear at general city coordinates, which can create misleading clusters. Use the "Remove unclear addresses" filter to see only verified locations.
    * The map represents a snapshot of what was published each year. Businesses may have opened or closed between editions, and Damron's information could be out of date.
    * These guides were primarily aimed at gay men, so lesbian spaces and other queer venues may be underrepresented. We are working on adding new layers of data from other guidebooks.

### The Amenities Visualization
* What it does: The amenities page provides interactive charts that show how different amenity features changed over time. You can compare up to three amenities simultaneously, view data as percentages or raw counts, and see geographic distributions on an integrated map.
* Best used for:
    * Tracking trends in how LGBTQ spaces were categorized over time (e.g., the rise of wheelchair-accessible venues, the presence of Black spaces, changes in women's spaces)
...
* Limitations:
    * This data is based on historical sources that require critical interpretation and must be read "against the grain." Understanding Bob Damron's perspective, business model, and the context in which these guides were created is essential. [Read more about Bob Damron and the history of the guides](#).
    * Amenity features reflect Damron's categorization system, which changed over time and carried its own biases (e.g., the use of "RT" for "Raunchy Types" conflated race, class, and morality)
    * The data shows what Damron reported, not objective measurements of spaces. Categories like "(B) - Blacks Frequent" or "(L) - Ladies" were based on impressions and reports, not demographic surveys.
    * Some amenity features appear or disappear over the years as Damron added or removed categories, making longitudinal comparisons challenging.
    * Percentage visualizations can be misleading when absolute numbers are small.

### The Locations Database
* What it does: The database browser allows you to search, filter, and sort through all locations in our dataset. You can search by keywords, filter by year, state, or amenity, and sort by different fields. Each entry shows the location's name, description, address, and all associated amenity features.
* Best used for:
    ...
    * Exporting lists of locations that meet specific criteria for further research
* Limitations:
    ...
    * The database is most useful when you know what you're looking for or are conducting systematic research.
...
First, Bob Damron’s Address Book primarily reflected commercial gay male culture, with less coverage of other queer spaces. Spaces for lesbians or other groups may be underrepresented. 
...
We believe in open source data and making our work freely available to researchers, educators, students, and the public. Our complete dataset and code are available through our GitHub repository.

### Accessing the Data

All of   our data is available at github.com/MappingtheGayGuides/MGG-Data 
