...
function gentable(querycriteria){
  ...
  tquery.where(querycriteria).run(function (error, tabledeets, response) {
    ...
    for (i=0; i < tabledeets.features.length; i++){
      counter = i + 1
      var row = [];
      row.push(tabledeets.features[i].properties.title, tabledeets.features[i].properties.description);
      table.row.add( {
              "name":       "Tiger Nixon",
      ...
      // $('#mggdatatablebody').append("<tr><td>" + tabledeets.features[i].properties.title + "</td><td>" + tabledeets.features[i].properties.description + "</td><td>" + tabledeets.features[i].properties.Year + "</td></tr>");
